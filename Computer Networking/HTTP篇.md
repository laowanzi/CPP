...
从 HTTP 协议的角度，服务器不会主动给客户端发消息。

> 那些看起来服务器主动发消息给客户端的场景，背后都是用什么机制实现的？
...
HTTP请求将超时设置的很大，这样就减少了请求的次数，并且后端能立刻将消息返回给前端。

例如百度云网盘

以上的两种方案，本质上还是客户端主动去取数据，只能应付简单场景。

### WebSocket介绍

全双工：同一时间里，双方都可以主动向对方发送数据，就是所谓的全双工。

TCP 协议是全双工的，HTTP 协议是半双工的，同一时间只能有一方发送数据。

### 如何建立WebSocket连接

不管是使用 HTTP 协议还是 WebSocket 协议，为了兼容使用场景，
...
`101 Switching Protocols`：101 状态码指**协议切换**
